...
- **Catalogue refresh** — `flexrouter refresh` (`flexrouter/refresh.py`: `refresh_config`), querying each configured provider's model list and comparing it against the settings file. It only records what it found — models that appeared, models that vanished, rate limits or context windows that changed — to `state/catalog_pending.json`, keyed by provider with when each was checked. It never writes `config.yaml` or `overrides.json`; nothing is applied until an owner-facing acceptance step exists (spec §6, a later stage). `RefreshResult` (`added`/`removed`/`changed`) describes these same findings, not anything applied.
